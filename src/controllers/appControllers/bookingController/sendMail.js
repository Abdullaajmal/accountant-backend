...
const { SendBooking } = require('@/emailTemplate/SendEmailTemplate');
...
const mail = async (req, res) => {
  try {
    ...
    const fileId = 'booking-' + booking._id + '.pdf';
    const folderPath = 'booking';
    ...
    const clientName = booking.client?.name || booking.client?.company || 'Customer';
    const subject = `Booking Confirmation ${booking.bookingNumber} - ${settings['idurar_app_name'] || 'Your Company'}`;
    ...
    return res.status(200).json({
      success: true,
      result: result.data,
      message: `Booking confirmation sent successfully to ${clientEmail} via ${result.method}`,
    });
  } catch (error) {
